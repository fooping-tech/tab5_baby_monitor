...
- The snapshot handoff alone did not remove the reported flicker. The working `tab5_rtsp_logger` rebinds the LVGL canvas buffer for every completed snapshot, which drops the image cache before invalidation, and allocates both canvas buffers as 8-bit-addressable PSRAM. Align this project with that publish sequence and memory capability before the next attended display check.
- Rebound the canvas buffer before every invalidate and changed both preview buffers to 8-bit-addressable PSRAM, then rebuilt and flashed the Tab5. `git diff --check`, host CTest 3/3, Python unittest 6/6, and the ESP-IDF P4 build passed. A 35-second post-flash serial capture showed `preview_frames` advancing from 24 to 121 with a USB UVC RTSP initialization; no reset, panic, or watchdog record appeared in that capture. Two MJPEG decode timeouts were safely rejected. The physical display still requires attended confirmation that the cyan intermediate frame no longer appears; this short capture is not a long-run stability result.
